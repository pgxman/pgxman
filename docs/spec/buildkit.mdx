...
A pgxman devpack is a configuration file in YAML format that `pgxman` uses to specify how a PostgreSQL extension should be built and packaged. devpacks are added to the [devpack repository](https://github.com/pgxman/devpack).

Below is an example devpack configuration including all the fields:

```yaml
# API version of the devpack specification. Only v1 is supported.
apiVersion: v1
...
- **Description**: Defines the API version to which the devpack conforms, ensuring its compatibility.
- **Type**: String
- **Required**: Yes
...
- **Description**: Lists the individuals or organizations responsible for maintaining the devpack.
...
- **Description**: Specifies the version of the extension devpack. The versioning must adhere to the [Semantic Versioning 2.0.0](https://semver.org) format.
- **Type**: String
- **Required**: Yes
